...
**已發布的頁面沒有草稿版可言。** WordPress 沒有這個功能——把一個已發布的頁面
設成草稿，是直接把它從網站上撤下來，不是做一份預覽。工具會擋住這個動作，
除非你另外加 `--unpublish` 明講你就是要下架。

所以已發布內容的安全網不是草稿，是這三層：

1. **`diff`** ——推之前先看清楚要改什麼。這是最重要的一層
2. **後台的修訂版本** ——推上去之後不滿意，一鍵還原
3. **`content/_backup/`** ——每次推送前自動存的線上快照

`draft` 真正的用途是**還沒發布過的東西**：用 `./wp.py new posts "標題" --yes`
建立的新文章，寫作期間就一直是草稿，寫完再 `push` 發布。
...
| 檔案 | 內容 | 能不能改 |
| --- | --- | --- |
| `content/pages-12-about.html` | 內文 | 可以 |
| `content/pages-12-about.json` | `title` 標題、`excerpt` 摘要 | 可以 |
| 同上，底線開頭的欄位 | `_id`、`_slug`、`_status`… | 只是參考，改了不會生效 |

**網址（`_slug`）刻意不能從這裡改。** 改網址會讓所有既有連結、書籤、
搜尋結果全部指向 404。真要改請到後台改，而且要自己決定設不設轉址。

### 新增一篇文章

```bash
./wp.py new posts "雨水過後的茶山" --yes
```

會建立一篇草稿並拉到本機。草稿不會出現在網站上，寫完再 `push` 發布。
...
想更嚴格的話，把 `pull` 也移到 `ask` 區塊。三個寫入指令都留在 `ask`
是刻意的——`draft` 用錯地方會讓頁面下架，它不比 `push` 安全。
