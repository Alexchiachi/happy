...
(function () {
  ...
  const T = (key) => ({
    ...
    formOk:       { tw: '收到了。確認信已寄到您的信箱，我們會在三個工作日內回覆。',
                    cn: '收到了。确认信已发到您的邮箱，我们会在三个工作日内回复。' },
  ...
  // --- Letter form → Cloudflare Worker（和幸福餐桌課程頁共用；見 executive-table/worker/letters.js）---
  // 來信存進 D1，通知寄到 dadaoissimple@gmail.com，並寄一封收信確認給寫信的人；在 /admin「大道至簡來信」分頁查看。
  // 留空時表單「不會」假裝寄出，而是請訪客改用 Email —— 寧可麻煩，也不要讓來信憑空消失。
  const FORM_ENDPOINT = 'https://executive-table.jianchiachi.workers.dev/api/letter';
  ...
  if (letterForm) {
    ...
    letterForm.addEventListener('submit', (e) => {
      ...
      data.append('lang', LANG === 'cn' ? 'zh-Hans' : 'zh-Hant');
      ...
      // 用 FormData 送出（multipart），屬於「簡單請求」，瀏覽器不必先發 CORS 預檢。
